...
The permanent project record is live at [atlas.yuvrajkashyap.com](https://atlas.yuvrajkashyap.com), and the source is public at [YuvrajKashyap/atlas](https://github.com/YuvrajKashyap/atlas). The operational console intentionally fails closed while the expiring AWS runtime is offline.
...
The permanent Vercel site and Edge Config contract are live and verified in the intentional `offline` state. The deterministic 10,000-page fault benchmark passed all release invariants and published its checked artifact from commit [`cedd633`](https://github.com/YuvrajKashyap/atlas/commit/cedd63358985fbae7ceff8b8041a90d40657f3ba). CI, container scanning, SBOM generation, CodeQL, backend coverage gates, Terraform validation, and Checkov are green. The remaining hard gates require one expiring AWS rehearsal: the authenticated live-console journey, timed recovery exercise, clean-checkout runtime recreation, teardown audit, and final release recording/tag.
